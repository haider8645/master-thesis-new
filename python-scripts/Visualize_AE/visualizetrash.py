...
model_def = '/home/lod/master-thesis/examples/master-thesis/trashnet/train-CAE02trashset.prototxt'
model_weights = '/home/lod/master-thesis/examples/master-thesis/trashnet/snapshots_iter_20000.caffemodel'
...
dirname = '/home/lod/master-thesis/graphs/output_trash'
#os.mkdir(dirname)
for j in range(10):
#os.path.join(dirname, face_file_name)
    net.forward()
    for i in range(1):
        cv2.imwrite(os.path.join(dirname,'input_image_' + str(j) + '.jpg'), net.blobs['train'].data[0,i])

    for i in range(1):
        cv2.imwrite(os.path.join(dirname,'output_image_' + str(j) + '.jpg'),net.blobs['deconv1neur'].data[0,i])
